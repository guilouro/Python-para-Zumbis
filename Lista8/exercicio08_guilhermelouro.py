...
dsgnhsf kgj mfm vjmmf wptc rtb dtblv frp wwmngbk dxss txz fzlxll lbqjrp hhcxnnhn skhcwxw 
lllkssb dgwd lzq czhhtclr bpngkf krh bzccxd rqkr mdz phk dfctpgr gkt bdnfbh wlj 
...
def preposicao(texto):
	count = 0
	for p in texto:
		if p[0] in tipo_zumbi and p[len(p)-1] not in tipo_zumbi:
			count += 1
	return count
...
def verbos(texto):
	...
	verbos_p_pessoa = 0
	for p in texto:
		if len(p) == 7 and p[len(p)-1] not in tipo_zumbi:
			verbos += 1
			if p[0] not in tipo_zumbi:
				verbos_p_pessoa += 1
	return (verbos, verbos_p_pessoa)


def ordem_alfabetica(texto):
	return sorted(texto, key=lambda x: [regra.index(i) for i in x])



def numeros_magicos(texto):
	n_magicos = []
	for palavra in texto:
		n = sum((lambda x: [regra.index(x[i])*20**i for i in range(0, len(x)) ] )(palavra))
		if not n%42 and distintos(palavra):
			n_magicos.append(palavra)
	return n_magicos


def distintos(palavra):
	for p in palavra:
		if palavra.count(p) > 1:
			return False
	return True
